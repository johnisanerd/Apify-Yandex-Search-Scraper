...
Yandex API выполняет поиск в Яндексе по любому запросу и возвращает чистый структурированный JSON. Каждая страница результатов содержит органическую выдачу (заголовок, ссылка, описание, позиция, быстрые ссылки), платную рекламу, карточки графа знаний, блоки изображений и видео, а также метаданные по странице. Поддерживаются более 15 доменов Яндекса, 19 языков и таргетинг по более чем 123 000 регионам, с автоматической пагинацией и управлением безопасным поиском.
...
**Создан для русскоязычных рынков и стран СНГ.** Поддерживает более 15 доменов Яндекса (yandex.com, yandex.ru, yandex.com.tr и другие), 19 языков и более 123 000 идентификаторов регионов. Это удобно для SEO, конкурентной разведки и мониторинга бренда в России, Восточной Европе, Центральной Азии и за их пределами.
...
|-----------|------|----------|---------|-------------|
| `text` | `string` | Да | - | Поисковый запрос. Поддерживает операторы Яндекса, например `date:YYYYMMDD..YYYYMMDD`. |
| `yandex_domain` | `string` | Нет | `yandex.com` | Домен Яндекса, например `yandex.ru`, `yandex.com.tr`, `yandex.kz` (поддерживается 15+). |
...
| `max_pages` | `integer` | Нет | `2` | Максимум страниц (`0` = без лимита). |
...
Пример элемента набора данных для запроса `Apple`. Каждая страница результатов - один элемент; массивы и часть полей сокращены для читаемости.
